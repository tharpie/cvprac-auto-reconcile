...
def get_devices(client):
    '''
    Function to gather all devices into a single list
    Interates over each container and appends devices for every node
    '''
...
def validate_and_compare_configlets(client, device_id, configlet_keys):
    '''Uses the validateAndCompareConfiglets.do API call to run compliance for a node'''
...
def update_add_reconcile(client, name, config, key, device_id):
    '''Uses the updateReconcileConfiglet.do API call to update the reconcile configuration'''
...
def cancel_pending_tasks(client):
    '''
    Method for getting all tasks that are in Pending status
    This function only cancels tasks that were generated by auto-reconcile
    '''
...
def reconcile(client, device):
    ...
    '''Verify that a RECONCILE configuration exists for a device, if not set to None'''
    try:
    ...
    '''
    Gather all the configlets for a given node
    
    If reconcile is applied, mark reconcile_applied == True
    this determines if we need to apply the reconile later

    The reconcile configlet isn't added to the list of configlet_keys
    This is the cleanest way to ensure the RECONCILE is always up to date
    '''
    ...
    try:
        ...
        print 'CvpApiError occured during validate/compare configlets...Error Message:'
        print e.msg
    ...
    if result['reconcile'] > 0:
        ...
        '''If no reconcile key exists, generate the key based on reconcile config'''
        ...
        '''Applies reconcile to device if it currently isn't applied'''
...
def main():
    ...
    ### Verify that a valid CVP node connection can be made
    try:
    ...
    for device in devices:
        ...
        if result['complianceCode'] != '0000':
            ...
            result = reconcile(cvp_client, device)
            if result:
